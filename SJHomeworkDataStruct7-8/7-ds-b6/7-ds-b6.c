...
void MergeList_L_SaveLaLb(LinkList *La, LinkList *Lb, LinkList *Lc)
{
	LinkList pa = (*La)->next, pb = (*Lb)->next, pc, newNode;
	if (*Lc)
	{
		DestroyList(Lc);
	}
...
int main()
{
	const int data_LA[] = { 1, 1, 3, 4, 5, 9 };
	const int data_LB[] = { 1, 2, 6, 11, 11 };
	LinkList LA, LB, LC;
	InitList(&LA);
	InitList(&LB);
	InitList(&LC);
