...
Status MyAlloc(LinkList *p, ElemType e)
{
	*p = (LinkList)malloc(sizeof(LNode));
#if defined (ELEMTYPE_IS_CHAR_P)
	(*p)->data = (ElemType)malloc((strlen(e) + 1) * sizeof(char));
	if ((*p)->data == NULL)
		return LOVERFLOW;
#elif defined (ELEMTYPE_IS_STRUCT_STUDENT_P)
...
int main()
{
	...
	for (i = INSERT_NUM * 2; i>0; i -= 2)
		ListInsert(&L, 1, (ElemType)i, MyAssign, MyAlloc);	//i是整型，严格来说需要转换为double
#elif defined (ELEMTYPE_IS_CHAR_ARRAY)
	printf("插入%d个元素：\n", INSERT_NUM);
	for (i = INSERT_NUM * 2; i>0; i -= 2) {
		sprintf(e1, "test%03d", i);
		ListInsert(&L, 1, e1, MyAssign, MyAlloc);
	}
	...
	printf("插入%d个元素：\n", INSERT_NUM);
	for (i = INSERT_NUM * 2; i>0; i -= 2)
		ListInsert(&L, 1, i, MyAssign, MyAlloc);
#endif
